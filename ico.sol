...
contract BanqiroTokenICO is Ownable {
    ...
    uint256 public phase0Supply = 3840000000000000000000000;
	uint256 public phase1Supply = 4800000000000000000000000;
	uint256 public phase2Supply = 8640000000000000000000000;
	uint256 public phase3Supply = 12000000000000000000000000;
	uint256 public phase4Supply = 18720000000000000000000000;
	...
	function getLevelsUnlocked(address user) public view returns(uint256 levels){
		...
		for(uint256 i=0; i< totalActivePartners; i++){
			if(usdInvestedByUser[Referal(referalContract).getAllRefrees(user)[i]] >= unlockPrice ){
                levels++;
			}
		}
	}
